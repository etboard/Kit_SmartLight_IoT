...
//------------------------------------------------------------------------------------------
#define NORMAL_SEND_INTERVAL  (1000 * 5)          // 2초 (단위: 초/1000)
...
{ 
  ...
  app.mqtt.publish_tele("/brightness", String(cds));   // 조도 값을 송신
}
  ...
  app.mqtt.client.subscribe(
    ...
    [&](const String & payload) {                 // 명령 내용을 payload에 저장       
      ...
      if (payload == "automatic"){                // 자동모드 설정 명령이면
        ...
        digitalWrite(D5, HIGH);                   // D5 핀을 HIGH로 설정 D5 LED 켜기
      }
      else{
        app.operation_mode = "manual";            // 수동모드로 저장
        digitalWrite(D5, LOW);                    // D5 핀을 HIGH로 설정 D5 LED 끄기
        }
      });
}
